...
function ResponsiveDrawerComp(props) {
  ...
  const drawer = (
      <div>
        ...
        <Divider />
        <List>
          {['Home', 'Plants', 'Water PH Level', 'Light Meter', 'History'].map((text, index) => (
            <ListItem key={text} disablePadding>
              <ListItemButton component={Link} to={listIcons[text.replace(/\s/g, "")].link} selected={tabIndex === listIcons[text.replace(/\s/g, "")].index} onClick={(event) => {handleTabIndex(event, listIcons[text.replace(/\s/g, "")].index)}}  disableRipple={true}>
                <ListItemIcon>
